...
@dlt.table(name="blocks", comment="NEAR Lake Testnet Raw Blocks")
def testnet_blocks():
    return (spark.readStream.format("cloudFiles")
      .option("cloudFiles.format", "json")
      .option("cloudFiles.schemaLocation", '/nearlake/landingZone/testnet_blocks/_checkpoint')
      .option("cloudFiles.inferColumnTypes", "true")
      .option("cloudFiles.useIncrementalListing", "true") 
      .option("pathGlobFilter", "*block.json") 
      .load("s3a://near-lake-data-testnet/"))
    
@dlt.table(name="chunks", comment="NEAR Lake Mainnet Raw Chunks")
def testnet_chunks():
    return (spark.readStream.format("cloudFiles")
      .option("cloudFiles.format", "json")
      .option("cloudFiles.schemaLocation", '/nearlake/landingZone/testnet_chunks/_checkpoint')
      .option("cloudFiles.inferColumnTypes", "true")
      .option("cloudFiles.useIncrementalListing", "true") 
